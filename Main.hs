...
showMove ((r0, c0), (r1, c1)) = do
    elDynAttrNS' svgNamespace "line" 
         (constDyn $  "x1" =: packShow ((fromIntegral c0 :: Float) + 0.5)
                   <> "y1" =: packShow ((fromIntegral r0 :: Float) + 0.5)
                   <> "x2" =: packShow ((fromIntegral c1 :: Float) + 0.5)
                   <> "y2" =: packShow ((fromIntegral r1 :: Float) + 0.5)
...
render board tour = do
    ...
    let moveMap = fmap (fromSet (const ()) . fromList . getMoves) tour 
    listWithKey moveMap (\c _ -> showMove c)

    return $ leftmost checkerEv


outlineStyle :: Int -> Map Text Text
outlineStyle width = "style" =: pack ("width: " ++ show width ++ "px; border: 5px solid navy;")

floatWithPaddingStyle :: Map Text Text
floatWithPaddingStyle = "style" =: "float:left;padding:10px" 

centerTextStyle :: Map Text Text
...
view width height rowCount colCount board tour = do
    let unvisited = fmap (\m -> pack $ "unvisited count = " ++ show (length board - length m)) tour
    ...
    elAttr "div" (outlineStyle width) $ do
        ...
        el "div" $ do
            (_, ev) <- elDynAttrNS' svgNamespace "svg" 
                            (constDyn $  "viewBox" =: pack ("0 0 " ++ show colCount ++ " " ++ show rowCount)
...
knightsTour :: MonadWidget t m => Int -> Int -> Int -> Int -> NominalDiffTime -> m ()
